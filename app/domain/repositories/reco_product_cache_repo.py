...
class RecoProductsCacheRepo:
    ...
    def __init__(self, redis, key_prefix: str):
        ...
        Args:
            redis: Redis client instance (can be None if disabled)
    ...
    async def get(self, key: str) -> Optional[list[RecoItem]]:
        """Retrieve a list of RecoItem from cache, returns None if not found or cache disabled."""
        if not self.cache:
            return None
    ...
    async def set(self, key: str, items: Iterable[RecoItem], ttl: int) -> None:
        """Store a list of RecoItem in cache, no-op if cache disabled."""
        if not self.cache:
            return
